...
创建型模式，共五种：工厂方法模式、抽象工厂模式、单例模式、建造者模式、 <a href='#原型模式'>原型模式</a> 。

结构型模式，共七种：适配器模式、装饰器模式、代理模式、外观模式、桥接模式、组合模式、  <a href='#享元模式'>享元模式</a> 。

行为型模式，共十一种：策略模式、模板方法模式、<a href='#观察者模式'>观察者模式</a>、迭代器模式、责任链模式、命令模式、备忘录模式、状态模式、访问者模式、中介者模式、解释器模式。
...
```
 ...
 ## <span name='原型模式'>原型模式 </span>  语言之魂 （五）
...
var address=(function(){
 ...
 + 命令模式实际是回调函数的一个面向对象的替代品，将请求与实现解耦并封装成独立的对象，从而使不同的请求对客户端的实现参数化。
...
+  抽象出一个对象来 ，具体例子是茶和咖啡
+  严重依赖抽象类的设计模式  JavaScript 在语言层并没有提供对抽象类的的支持。
...
//  init 就是模板 
 ...
 Baverage.prototype.boliWater=function(){
 	console.log('把水煮沸')
 };
...
##  迭代器模式 （八 ）
+ 是提供一种方法顺序访问一个聚合对象的内部表示，迭代器模式将迭代的过程从业务逻辑中分离出来。
+ 内部迭代器和外部迭代器 
 ...
 var compare=function(Iterator1,Iterator2){
 	while(!Iterator1.isDone()&& !Iterator2.isDone()){
 		if(Iterator1.getCurrItem()!==Iterator2.getCurrItem()){
 			throw new Error('Iterator2和Iterator2 不相等');

 		};
 			Iterator2.next();
 			Iterator1.next();
 	};
 	console.log('Iterator2和Iterator2 相等')

 };
 var iterator1=Iterator([1,3,2]);
 var iterator2=Iterator([1,2,3]);
compare(iterator2,iterator1)
...
var getFormUploadObj=function(){
 ...
 var uploadObj=interatorUploadObj(getActiveUploadObj,getFlashUpload,getFormUploadObj)//如果后期有新的就再添加
...
+ 在不破坏对象的封装的前提下，在对象之外捕获并保存该对象内部的状态以便日后对象使用或者对象恢复到以前的某个状态

+ 备忘录模式最主要的任务是对现有的数据和状态做缓存，为将来某个时刻使用或者恢复做准备
...
+  针对 于对象结构中的元素，定义在不改变该对象的前提下访问结构中元素的新方法
+  访问者模式 解决数据与数据的操作方法之间的耦合，将数据的操作方法独立于数据，使其可以自由化演变，因此访问者更适合于那些数据稳定，但是数据的操作方法
易变得环境下。
+  当环境操作环境改变时，可以自由修改操作方法以适应操作环境，而不是修改原数据，实现操作方法的拓展。同时对于同一个数据源，它可以被多个访问对象所访问，这极大增大了操作数据的灵活性

```js

绑定IE 事件
function bindIEEvent(dom,type,fn,ddta){
 	var data=data||{};
 ...
  var  Visitor=(function(){
  	return {
  		splice:function(){
  			var args=Array.prototype.splice.call(arguments,1);//参数从原参数的第二个参数开始 算起 
  			return  Array.prototype.splice.apply(arguments[0],args); // 对第一参数对象执行splice方法 

  		},
  		push:function(){
  			var len=arguments[0].length||0; // 	强化类数组对象使他拥有length 属性 
  			var  args=this.splice(arguments,1)// 添加的数据从原参数的第二个参数算起
  			arguments[0].length=len+arguments.length-1;//重新计长度
  ...
  Visitor.push(a,1,2,3,4,5,6);
  console.log(a.length)//6
  console.log(a);
  Visitor.push(a,7,8,9);
...
+ 对于一种语言给出 其文法表示形式，并定于一种解释器 通过使用这种解释器来解释语言中定义的句子

+ 文法就是一种语言中的语言描述的工具，比如我们汉化中的疑问句'什么什么吗'，感叹句‘什么什么啊！’ 文法是用来定义一组语言规则的
...
```js 
    	...
    	if(node.previousSibling){
    		...
    		while(sibling){
    			if(sibling.nodeType==1&&sibling.nodeType===node.nodeType&&sibling.nodeName){
    				//如果节点是元素并且类型与钱一个兄弟元素的类型相同，并且前一个兄弟元素名称存在
    ...
    var  Interpreter=(function(){
    	...
    	// 	// .......
    	// };
    	// 参数1 node:目标节点参数二 wrap :容器节点
    	return function(node,wrap){
    		var path=[];
    		wrap=wrap||document;
    		if(node===wrap){
    			if(wrap.nodeType==1){ //  1代表的是Element 节点  8 代表的是comment节点   11  代表的是documentFragment 节点    9 代表的是 document  节点
    		...
    		if(node.nodeType==1){
    			path.push(getSublingNames+node.nodeName.toUpperCase())
    ...
    var  path=Interpreter(span7);
    var  sub=getSublingName(div1)
    console.log(path);//["HTML", "|HEADBODY", "2DIV", "2DIV", "UL", "4LI", "SPAN"]
    console.log(sub)
